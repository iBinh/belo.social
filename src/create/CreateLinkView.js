...
export class CreateLinkView extends TemplateView {
    render(t, vm) {
        ...
        return t.div({className: "CreateLinkView card"}, [
            t.h1("Create shareable links to Belo Chat"),
            t.form({action: "#", onSubmit: evt => this._onSubmit(evt)}, [
                t.div(t.input({
                    ...
                    name: "identifier",
                    required: true,
                    placeholder: "#room:chat.belo.chat, @user:chat.belo.chat",
    ...
    _onIdentifierChange(evt) {
        ...
        if (!this.value.validateIdentifier(inputField.value)) {
            inputField.setCustomValidity("That doesn't seem valid. Try #room:chat.belo.chat, @user:chat.belo.chat or +group:chat.belo.chat.");
        } else {
